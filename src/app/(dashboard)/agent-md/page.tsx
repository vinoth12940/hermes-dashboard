...
export default function AgentMDPage() {
  ...
  const fetchFile = async () => {
    try {
      const res = await fetch('/api/agent-md');
      if (res.ok) {
      ...
      setSaveMessage('Failed to load AGENTS.md');
    }
    setLoading(false);
  };
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-64"><p className="text-zinc-500">Loading AGENTS.md...</p></div>;
  }
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="glass-card p-6">
        <textarea
          ...
          className="w-full h-[70vh] dark:bg-zinc-900/80 bg-zinc-50/80 dark:text-zinc-600 text-zinc-500 font-mono text-sm p-4 rounded-xl border dark:border-zinc-800/50 border-zinc-200/50 resize-none focus:border-indigo-500/50 leading-relaxed"
          spellCheck={false}
